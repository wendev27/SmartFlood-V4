...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (role !== "barangay") {
      return NextResponse.json({ success: false, error: "Only barangay users can reject barangay emergency allocations." }, { status: 403 });
    }
    ...
    if (item.barangay_status === "rejected") {
      return NextResponse.json({ success: true, data: { allocation_item: item, notification: await getItemNotification(item) }, already_rejected: true });
    }
    if (item.barangay_status !== "notified") {
      return NextResponse.json({ success: false, error: `Allocation status ${item.barangay_status} cannot be rejected.` }, { status: 400 });
    }
    ...
    if (notification.status === "accepted") {
      return NextResponse.json({ success: false, error: "An accepted notification cannot reject an allocation." }, { status: 400 });
    }

    const now = new Date().toISOString();
    const { data: updatedItem, error: itemError } = await supabaseServer
      .from("emergency_allocation_items")
      .update({ barangay_status: "rejected", accepted_at: null, rejected_at: now })
      .eq("item_id", itemId)
    ...
    const { data: updatedNotification, error: notificationError } = await supabaseServer
      .from("notifications")
      .update({ status: "rejected", read_at: notification.read_at ?? now, accepted_at: null, rejected_at: now })
    ...
    if (notificationError || !updatedNotification) {
      ...
      return NextResponse.json({ success: false, error: notificationError?.message ?? "Notification was already changed. Allocation rejection was rolled back." }, { status: notificationError ? 500 : 409 });
    }

    const batch = await getBatch(String(updatedItem.batch_id));
    await logAuditEvent({
      ...auditActorForViewer(viewer),
      action: "BARANGAY_ALLOCATION_REJECTED",
      module: "Emergency Relief Notification",
      description: `${barangay.barangay_name} rejected allocation item ${itemId} for batch ${String(updatedItem.batch_id)}: ${Number(updatedItem.family_food_packs)} food packs, ${Number(updatedItem.individual_relief_goods)} individual relief goods, ${Number(updatedItem.emergency_kits)} emergency kits.`,
    ...
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : "Unable to reject emergency allocation." }, { status: 500 });
  }
}
