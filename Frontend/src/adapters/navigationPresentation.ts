...
export function navigationPresentation(items: NavItem[], role?: DashboardRole, logLabel?: string): { primary: NavItem[]; groups: NavigationGroup[] } {
  ...
  if (role !== "super") return { primary: presented, groups: [] };
  ...
  const residentKeys: PageKey[] = ["residents", "accounts"];
  const groups = ([
    { label: "CSWDD", icon: "cube", items: presented.filter((item) => reliefKeys.includes(item.key)) },
    { label: "Barangay Services", icon: "users", items: presented.filter((item) => residentKeys.includes(item.key)) },
  ] satisfies NavigationGroup[]).filter((group) => group.items.length > 0);
  return {
    primary: presented.filter((item) => !reliefKeys.includes(item.key) && !residentKeys.includes(item.key)),
    groups,
  };
}
